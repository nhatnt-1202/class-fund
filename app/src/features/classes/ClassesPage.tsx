...
export default function ClassesPage() {
  ...
  if (!allowed) {
    return (
      <Card className="p-6">
        <Note tone="warn">
          <span>
            Chỉ tài khoản gốc của hệ thống mở được lớp mới. Bạn đang quản trị
            lớp của mình — mọi việc trong lớp (đợt thu, thu, chi, thành viên)
            làm ở các trang khác.
          </span>
        </Note>
      </Card>
    );
  ...
  const studentCount = (id: string) =>
    counts.data?.find((c) => c.class_id === id)?.student_count ?? 0;
  const adminsOf = (id: string) =>
    (admins.data ?? []).filter((a) => a.class_id === id);
  ...
  const submitAssign = async () => {
    ...
    try {
      ...
      toast.ok(
        r.status === "invited"
          ? "Đã tạo lời mời quản trị"
          : "Đã giao quản trị lớp",
        r.status === "invited"
  ...
  return (
    ...
    >
      <Note tone="info">
        <span>
          Bạn là <b>tài khoản gốc</b>: mở lớp và giao mỗi lớp cho một tài khoản
          quản trị. Quản trị lớp chỉ thấy và quản lý lớp được giao. Sinh viên có
          email{" "}
          <b>
            &lt;mã SV&gt;@
            {config?.student_email_domain ?? "student.humg.edu.vn"}
          </b>{" "}
          tự đăng ký và tự vào đúng lớp có mã sinh viên đó trong danh sách đã
          nhập.
        </span>
      </Note>
          ...
          <TableWrap>
            <table>
              <caption className="sr-only">
                Danh sách lớp và quản trị của từng lớp
              </caption>
              <thead>
                <tr>
                  <th>Mã lớp</th>
                      ...
                      <td className="text-[13px]">
                        {list.length === 0 ? (
                          ...
                          <span className="flex flex-wrap gap-1.5">
                            {list.map((a) => (
                              <Badge
                                key={`${a.class_id}-${a.email}`}
                                tone={a.pending ? "neutral" : "ok"}
                              >
                                {a.full_name || a.email}
                                {a.pending ? " · chờ đăng ký" : ""}
                              </Badge>
                            ))}
                          </span>
                        )}
      ...
      >
        ...
        >
          <Input
            ...
            placeholder="loptruong@student.humg.edu.vn"
